...
function get_gj (gj, token) {
    console.log('轨迹加密使用的token值:::',token)
    var gj_list = []
    for (var i of gj) {
      var gj_new = window.jjj(token, i + '')
      gj_list.push(gj_new)
    }
    return gj_list
}
//              滑块缺口距离，token值，轨迹数组
function get_hk_data(distance,token,track_List){
    var traceData = get_gj(track_List, token)  //轨迹加密
    var Z = window.yyy['sample'](traceData, 50) //截取加密后的轨迹前50个
    , H = ffff(jjj(token, parseInt(distance, 0xa) / 320 * 100 + ''))
    , f0 = GGG(window.yyy['unique2DArray'](track_List, 0x2));

    var data = {
        'd': ffff(Z['join'](':')),
        'm': '',
        'p': H,
        'f': ffff(jjj(token, f0['join'](','))),
        'ext': ffff(jjj(token, 1 + ',' + track_List['length']))
    };
