...
function main(){
    ...
    // // 关键：从指令列表中提取所有需要执行的指令ID（去重）
    // const usedInsts = new Set();
    // compiler.instList.forEach(inst => {
    //     // 只保留数字指令（过滤标签引用等非指令值）
    ...
    // });
    const usedInsts = new Set();
    const instList = compiler.instList;
    let pc = 0; // 指令计数器（指针）
    const PUSH_INST = 101; // 双字节指令：PUSH（需跳过后续参数）

    while (pc < instList.length) {
        const current = instList[pc];
        // 只处理数字（排除非指令值，如标签引用的占位符，实际genInst后已为数字）
        if (typeof current === 'number') {
            if (current === PUSH_INST) {
                // PUSH是双字节指令：当前是指令ID，后续是参数，需跳过参数
                usedInsts.add(current);
                pc += 2; // 跳1步参数
            } else {
                // 单字节指令：当前是指令ID，无参数
                usedInsts.add(current);
            ...
            // 非数字（如未处理的标签对象，实际genInst后不会出现），直接跳过
            pc += 1;
        }
    }
    console.log("需要执行的指令ID：", [...usedInsts]);

    const caseRegex = /case (\d+):\s*\{[\s\S]*?break;\s*\}\s*/g;
    let filteredVmCode = vmCode.replace(caseRegex, (match, instId) => {
        // 只保留 usedInsts 中存在的指令case
        return usedInsts.has(Number(instId)) ? match : '';
    });
        vmCode = vmCode
    // 移除单行注释（// ...）
    .replace(/\/\/.*$/gm, '')
    ...
    // 3. 清理冗余代码（空行、多余大括号、空case残留）
    vmCode = filteredVmCode
        .replace(/(\n\s*){2,}/g, '\n') // 合并连续空行
        .replace(/\{\s*\}/g, '{ }') // 处理空大括号（避免语法错误）
        // .replace(/case \d+:\s*\{[\s\S]*?\}/g, match => { // 二次过滤未匹配到break的无效case
        //     return match.includes('break;') ? match : '';
        // });
    ...
    // console.log(jsCode)
    fs.writeFile("./output.js", `${vmCode}(${JSON.stringify(compiler.instList)}, ${JSON.stringify(compiler.poolList)});
console.log('还原后的代码为：\\n' + generator(allBody, {compact:false,   jsescOption: {minimal: true,unicode: false}, format:{indent:{style:'  '}}}).code);`, (e)=>{});
}
main()

// console.log(generator(allBody, {compact:false, format:{indent:{style:'  '}}}).code);