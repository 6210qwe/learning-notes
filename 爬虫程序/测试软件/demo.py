...
class BrainyQuiz(QMainWindow):
    ...
    def add_question_dialog(self):
        ...
        q_type_combo.addItems(["填空题", "问答题", "单选题", "多选题", "判断题"])
        ...
        def on_type_change(index):
            ...
            if selected_type == "单选题":
                ...
                # 只显示第一个选项框作为填空输入
                self.option_edits[0].show()
                self.option_edits[0].parent().show()
                self.option_edits[0].setPlaceholderText("请输入填空题答案")
            elif selected_type == "问答题":
                # 隐藏所有选项框，改用大文本框
                options_group.hide()  # 隐藏整个选项组
                # 如果需要，可以在这里动态添加一个QTextEdit作为答题区域
