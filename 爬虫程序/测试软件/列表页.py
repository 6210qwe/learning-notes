        ...
        if next_data_script:
            ...
            for company in company_list:
                ...
                if not company.get('id'):
                    continue
                # 公司唯一标识ID
                item_data['company_id'] = company.get('id')
                item_data['data_source'] = '天眼查'

                # 公司名称（去除em标签）
                company_name = company.get('name') or ''
                item_data['company_name'] = company_name.replace('</em>', '').replace('<em>', '')

                # 公司所在地（基础信息）
                item_data['province'] = company.get('base')

                # 法定代表人姓名
                item_data['legal_representative'] = company.get('legalPersonName')

                # 成立时间（可能为简写格式）
                item_data['establishment_date'] = company.get('estiblishTime')

                # 注册资本
                item_data['capital'] = company.get('regCapital')

                # 注册状态（如：存续、注销等）
                item_data['companyStatus'] = company.get('regStatus')

                # 曾用名列表
                item_data['historyNames'] = company.get('historyNames')

                # 行业分类代码
                item_data['category_code'] = company.get('categoryCode')

                # 注册号
                item_data['reg_number'] = company.get('regNumber')

                # 组织机构代码
                item_data['orgCode'] = company.get('orgNumber')

                # 统一社会信用代码
                item_data['creditNo'] = company.get('creditCode')

                # 经营范围
                item_data['businessScope'] = company.get('businessScope')

                # 注册地址
                reg_location = company.get('regLocation') or ''
                item_data['companyAddress'] = reg_location.replace('</em>', '').replace('<em>', '')

                # 联系电话列表
                item_data['phoneNumbers'] = json.dumps(company.get('phoneList'), ensure_ascii=False)

                # 主要联系电话
                item_data['phone'] = company.get('phoneNum')

                # 所在城市
                item_data['city'] = company.get('city')

                # 处理邮箱列表
                email_list = company.get('emailList', [])
                item_data['emails'] = json.dumps(email_list, ensure_ascii=False)

                # 如果有邮箱，取列表第一个值给email字段
                if email_list:
                    item_data['email'] = email_list[0]
                else:
                    # 可以根据需要设置默认值，如空字符串
                    item_data['email'] = ""

                # 官方网站列表
                item_data['website'] = company.get('websites')

                # 公司别名/简称
                item_data['alias'] = company.get('alias')

                # 社保参保人数
                # item_data['social_security_staff_num'] = company.get('socialSecurityStaff_num')

                # 法定代表人（职位信息中的值）
                item_data['legal_representative'] = company.get('firstPositionValue')

                # 注册资本（职位信息中的值）
                item_data['registered_capital'] = company.get('secondPositionValue')

                # 公司B端唯一标识
                item_data['gid_for_b'] = company.get('gidForB')

                # 成立时间（标准格式）
                item_data['establishDate'] = company.get('establishmentTime')

                # 登记机关
                item_data['authority'] = company.get('registerInstitute')

                # 基本信息摘要
                item_data['introduction'] = company.get('abstractsBaseInfo')

                # 年报员工人数
                item_data['staff_num_report_year'] = company.get('staffNumReportYear')

                # 公司标签列表（V3版本）
                label_list = company.get('labelListV3', [])
                item_data['tags_list'] = json.dumps([
                    label.get('profileTagNameOnPage', '')
                    for label in label_list
                    if isinstance(label, dict)  # 确保每个元素是字典，避免类型错误
                ], ensure_ascii=False)

                # 所在省份名称
                item_data['province'] = company.get('provinceName')

                # 所在城市名称
                item_data['city'] = company.get('cityName')

                # 所在区/县名称
                item_data['district'] = company.get('districtName')

                # 一级行业分类名称
                # item_data['industry_classification'] = company.get('categoryNameLv1')
                item_data['industry'] = company.get('categoryNameLv1')

                # 二级行业分类名称
                item_data['industry_large_class'] = company.get('categoryNameLv2')

                # 三级行业分类名称
                item_data['industry_medium_class'] = company.get('categoryNameLv3')

                # 四级行业分类名称
                item_data['category_name_lv4'] = company.get('categoryNameLv4')

                # 企业类型
                item_data['companyType'] = company.get('orgType')

                # 核准日期
                item_data['issueDate'] = company.get('approveDate')

                # 经营期限
                item_data['operationEnddate'] = company.get('businessTerm')

                # 社保参保人数（标准字段）
                item_data['staff_num_report_year'] = company.get('socialSecurityStaffNum')

                # 公司规模（从规模信息中提取名称）
                company_scale_info = company.get('companyScaleInfo')
                if isinstance(company_scale_info, dict):
                    item_data['scale'] = company_scale_info.get('name')
                else:
                    item_data['scale'] = None
                # 标签页显示的英文名
                item_data['english_name'] = company.get('englishNameForTab')
                item_data['all_data'] = json.dumps(company, ensure_ascii=False)
                self.mysql_db.insert("company_info", data_dict=item_data, return_ids=True)
